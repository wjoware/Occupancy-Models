...
# changes:          
  ...
  # load R files from code developed by Matt & Tyler
    ...
    load("~/OneDrive - ucsc.edu/FISHBIO/Capstone Project/Occupancy Models/Approach 1B/2020/OccupancyData_09082021.Rdata")

  # create the right reference data repository
    setwd("~/OneDrive - ucsc.edu/FISHBIO/Capstone Project/Occupancy Models/Approach 1B/2020")
  ...
  # filter out unneeded columns
    stb.lt <- 
      cap.list.2020.ltSC$STB[, -c(1,2,3)] 
  ...
  # convert counts to 0s & 1s for detection
    ...
    sum(na.exclude(stb.lt))
      # calculate the total number of sampled striped bass = 11
  ...
  # Plot initial occupancy probability  & colonization probabilities across years
    ...
    # use predict function get initial occupancy, colonization, &
      ...
      # form dataframe for model predictions
        ...
        E.occ  <- predict(dm0, type = "psi", nd)
        E.col <- predict(dm0, type = "col", nd)
        E.det <- predict(dm0, type = "det", nd)
  ...
  # comparison of predicted occupancy & 
    # occupancy based on detections, or "naive" occupancy
      ...
      # adjust plot margins
        ...
        # plotting
          plot(pred.lt20$event, 
            pred.lt20$smoothed_occ, 
            ylim = c(0,1), 
               # plot probabilites along the y-axis from 0 to 1
            tck = 0.01,
            type = 'n', 
            ...
            xlab = "", 
            ylab = "Occupancy",
            main = "Small (FL < 300 mm) striped bass occupancy in 2020", 
            axes = F)
        ...
        # naive estimate of occupancy #
          # calculate proportions of occupied sites
            nums20lt <- apply(cap.list.2020.ltSC$STB[, -c(1:3)], 
                            2, function(x) length(which( x > 0)))
            ns20lt <- apply(cap.list.2020.ltSC$STB[, -c(1:3)], 
                          2, function(x) 39 - length(which( is.na(x))))
            props20lt <- nums20lt / ns20lt
        ...
        # plot points depicting estimates of actual & "naive" 
          ...
          # confidence intervals for "naive" estimates
            segments(pred.lt20$event, 
                   # specify where upper limit will be sourced
                  pred.lt20$smoothed_occ + pred.lt20$SE,
                   # upper
                  pred.lt20$event, 
