...
# -----------------------------------------------------------------------------
  # Alter initial occupancy, colonization, local extinction/persistence, 
    ...
    # functioning model for striped bass less than 300 mm fork length
# -----------------------------------------------------------------------------

# alter parameters by flow 
  # initial occupancy probability
  occ_m2 <-
    colext(~flow, # alter first-year occupancy probability by flow
         ~1, # constant colonization probability
         ~1, # constant extinction probability
         ~1, # constant detection probability
         stb.lt.UMF) # dataframe
...
det_m5 <-
  colext(~1, # constant first-year occupancy probability
         ...
         stb.lt.UMF) # dataframe
summary(det_m5) # results in logit scale
...
det_m6 <-
  colext(~1, # constant first-year occupancy probability
         ...
         stb.lt.UMF) # dataframe
summary(det_m6) # results in logit scale
...
det_m7 <-
  colext(~1, # constant first-year occupancy probability
         ...
         stb.lt.UMF) # dataframe
summary(det_m7) # results in logit scale
...
det_m8 <-
  colext(~1, # constant first-year occupancy probability
         ...
         stb.lt.UMF) # dataframe
summary(det_m8) # results in logit scale
...
det_m9 <-
  colext(~1, # constant first-year occupancy probability
         ...
         # river kilometer
         stb.lt.UMF) # dataframe
summary(det_m9) # results in logit scale
...
det_m10 <-
  colext(~1, # constant first-year occupancy probability
         ...
         stb.lt.UMF) # dataframe
summary(det_m10) # results in logit scale
...
#  Compare the  models  using AIC 
  det_AICs <-
    data.frame(Model = 
             ...
             AIC = 
               c(det_m1@AIC, det_m2@AIC, det_m3@AIC, det_m4@AIC, det_m5@AIC, 
                 det_m6@AIC, det_m7@AIC, det_m8@AIC, det_m9@AIC, det_m10@AIC))
  # model 3 (for striped bass less than 300 mm fork length) 
    # has the lowest AIC
    # recall model 6 changes detection by sampling day & river kilometer
    m6 <-
      colext(~1, # constant first-year occupancy probability
         ~1, # constant colonization probability
         ~1, # constant extinction probability
    ...
    # Backtransform model 6 estimates from logistic to  probability  scale 
      backTransform(m6, type =  "psi")   # First-year occupancy 
        # 8.16e03% chance of initial occupancy (SE = 0.28)
      backTransform(m6, type =  "col")   # Colonization probability 
        # 0.283% chance of colonization (SE = 0.106)
      backTransform(m6, type =  "ext")   # Extinction probability 
        # 1.97e04% chance of extinction (SE = 0.00614)
      ...
      lc <- linearComb(m6, c(1,4), type = "det")
        # make an object that stores detection from the model ("m4")
      backTransform(lc)
  ...
  # Quantitatively evaluate model fit
      library(AICcmodavg)
      gof_det <- mb.gof.test(m4, nsim = 1000, plot.hist = F, plot.seasons = T, report = 1)
      gof_det
