...
# changes:          
  ...
  # filter out unneeded columns
    stb.lt <- 
      cap.list.2019.ltSC$STB[, -c(1,2,3)] 
  ...
  # convert counts to 0s & 1s for detection
    ...
    sum(na.exclude(stb.lt))
      # calculate the total number of sampled striped bass
        # shorter than 300 mm fork lengths, excluding "NA" entries
          # 15 striped bass shorter than 300 mm fork lengths
    
    # ensure all "NA" entries align
      fishNAs <- which(is.na(stb.lt) == T)
    ...
    # ensure striped bass data is a matrix
      str(stb.lt) # data frame
      stb.lt <- as.matrix(stb.lt) # convert to a matrix
      str(stb.lt) # now a matrix
        
    # apply all NA positions from each variable to striped bass observations
      stb.lt[allNAs] <- NA
    
    # check whether NAs are now aligned
      length(which(is.na(stb.lt))) # striped bass observations
  ...
  # Explanation
    # SEPARATELY alter initial occupancy, colonization, local 
      # extinction/persistence, & detection probabilities by 
...
# ------------------------------------------------------------------------
  # Adjustments on Matt's suggested code for 
    ...
    # models
      dm0 <- colext( ~1, ~1, ~1, ~1, stb.lt.UMF)
        # null model
      dm1 <- colext( ~ 1, ~ 1, ~ 1, ~ flow, stb.lt.UMF)
        # alter detection probability by flow
      dm2 <- colext( ~ 1, ~ 1, ~ 1, ~ effort, stb.lt.UMF)
        # alter detection probability by effort
      dm3 <- colext( ~ 1, ~ 1, ~ 1, ~ flow + effort, stb.lt.UMF)
        # alter detection probability by flow & effort
      dm4 <- colext( ~ 1, ~ date, ~ 1, ~ flow, stb.lt.UMF)
        # alter colonization probability by sampling date &
          # detection probability by flow
      dm5 <- colext( ~ 1, ~ date, ~ 1, ~ effort, stb.lt.UMF)
        # alter colonization probability by sampling date &
          # detection probability by sampling effort
      dm6 <- colext( ~ 1, ~ date, ~ 1, ~ flow + effort, stb.lt.UMF)
        # alter colonization probability by sampling date &
          # detection probability by flow + sampling effort
      dm7 <- colext( ~ rkm, ~ date, ~ 1, ~ flow, stb.lt.UMF)
        # alter initial occupancy probability by river km,
          # colonization probability by sampling date, &
          # detection probability by flow
      dm8 <- colext( ~ rkm, ~ date, ~ 1, ~ effort, stb.lt.UMF)
        # alter initial occupancy probability by river km,
          # colonization probability by sampling date, &
          # detection probability by sampling effort
      dm9 <- colext( ~ rkm, ~ date, ~ 1, ~ flow + effort, stb.lt.UMF)
        # alter initial occupancy probability by river km,
          # colonization probability by sampling date, &
          # detection probability by flow + sampling effort
      dm10 <- colext( ~ rkm, ~ date, ~ date, ~ flow, stb.lt.UMF)
        # alter initial occupancy probability by river km,
          # colonization probability by sampling date, 
          # extinction probability by sampling date, &
          # detection probability by flow
      dm11 <- colext( ~ rkm, ~ date, ~ date, ~ effort, stb.lt.UMF)
        # alter initial occupancy probability by river km,
          # colonization probability by sampling date, 
          # extinction probability by sampling date, &
          # detection probability by sampling effort
      dm12 <- colext( ~ rkm, ~ date, ~ date, ~ flow + effort, stb.lt.UMF)
        # alter initial occupancy probability by river km,
          # colonization probability by sampling date, 
          # extinction probability by sampling date, &
          # detection probability by flow + sampling effort
    ...
    # develop the output that shows model comparisons
      ...
      # Extract values of interest from model summaries
        coef(dm2019_STBlt300_List) # coefficients 
        SE(dm2019_STBlt300_List) # standard errors
      
      # Model-averaged prediction
        predict(dm2019_STBlt300_List, type="psi")
          # predict initial occupancy probabilities by 
            # averaging estimates across all models
        predict(dm2019_STBlt300_List, type="col")
          # predict colonization probabilities by 
            # averaging estimates across all models
        predict(dm2019_STBlt300_List, type="ext")
          # predict extinction probabilities by 
            # averaging estimates across all models
        predict(dm2019_STBlt300_List, type="det")
...
# Visualize model results
  ...
  # Ploting method I
    ...
    # plotting settings  
      ...
      quartz(7,6) # plotting window size
      ...
      with(E.occ, 
             {
               plot(nd$rkm, 
                    ...
                    main = "2019 initial occupancy of small striped bass",
        ...
        with(E.col, 
             {
               plot(nd$date, 
                    ...
                    xaxt = "n", 
                    xlab = "Sampling day",
                    main = "2019 colonization of small striped bass",
        ...
        with(E.det, 
             {
               plot(nd$effort, 
                    ...
                    main = "2019 striped bass (Morone saxatilis) longer than 300 mm fork length",
  ...
  # Plotting method II
      ...
      # make prediction plots here w/ respect to covariates of top model AND
        # provide estimates and uncertainty of occupancy
          par(mfrow = c(2,2), # 2 rows & 2 columns
            mar = c(7,3,0.5,0.5)) # plot margin size settings
              # adjusts bottom, left, top, & right margins (in order)
    ...
    # comparison of predicted occupancy & 
      # occupancy based on detections, or "naive" occupancy
        ...
        # plotting
          plot(pred.lt19$event, 
               pred.lt19$smoothed_occ, 
               ylim = c(0,1), 
               # plot probabilites along the y-axis from 0 to 1
               tck = 0.01,
               type = 'n', 
               ...
               xlab = "", 
               ylab = "Occupancy",
               main = "Small (FL < 300 mm) striped bass occupancy in 2019", 
               axes = F)
          ...
          # naive estimate of occupancy #
            # calculate proportions of occupied sites
              nums19lt <- apply(cap.list.2019.ltSC$STB[, -c(1:3)], 
                            2, function(x) length(which( x > 0)))
              ns19lt <- apply(cap.list.2019.ltSC$STB[, -c(1:3)], 
                          2, function(x) 39 - length(which( is.na(x))))
              props19lt <- nums19lt / ns19lt
          ...
          # plot points depicting estimates of actual & "naive" 
            ...
            # "naive" occupancy (doesn't account for detection %)
              points(pred.lt19$event, 
                 c(mean( props19lt[1:2]), 
                   mean( props19lt[3:4]), 
                   mean( props19lt[5:6]), 
                   mean( props19lt[7:8])), 
          ...
          # confidence intervals for "naive" estimates
            segments(pred.lt19$event, 
                   # specify where upper limit will be sourced
                   pred.lt19$smoothed_occ + pred.lt19$SE,
                   # upper
                   pred.lt19$event, 
  ...
  # View proportion of occupied sites by secondary sampling events each year
              
    # small striped bass
      # 2019        
        num1s <- apply( cap.list.2019.ltSC$STB[, -c(1:3)], 
                        2, function(x) length(which( x > 0)) )
        ns <- apply( cap.list.2019.ltSC$STB[, -c(1:3)], 
                     2, function(x) 39 - length(which( is.na(x))) )
        props19lt <- num1s / ns
      # 2020         
        num1s <- apply( cap.list.2020.ltSC$STB[, -c(1:3)], 
                        2, function(x) length(which( x > 0)) )
        ns <- apply( cap.list.2020.ltSC$STB[, -c(1:3)], 
                     2, function(x) 39 - length(which( is.na(x))) )
        props20lt <- num1s / ns
      # 2021      
        num1s <- apply( cap.list.2021.ltSC$STB[, -c(1:3)], 
                        2, function(x) length(which( x > 0)) )
        ns <- apply( cap.list.2021.ltSC$STB[, -c(1:3)], 
                     2, function(x) 39 - length(which( is.na(x))) )
        props21lt <- num1s / ns
              
    # large striped bass
      # 2019
        num1s <- apply( cap.list.2019.gtSC$STB[, -c(1:3)], 
                        2, function(x) length(which( x > 0)) )
        ns <- apply( cap.list.2019.gtSC$STB[, -c(1:3)], 
                     2, function(x) 39 - length(which( is.na(x))) )
        props19gt <- num1s / ns
              
      # 2020
        num1s <- apply( cap.list.2020.gtSC$STB[, -c(1:3)], 
                        2, function(x) length(which( x > 0)) )
        ns <- apply( cap.list.2020.gtSC$STB[, -c(1:3)], 
                     2, function(x) 39 - length(which( is.na(x))) )
        props20gt <- num1s / ns
        
      # 2021
        num1s <- apply( cap.list.2021.gtSC$STB[, -c(1:3)], 
                        2, function(x) length(which( x > 0)) )
        ns <- apply( cap.list.2021.gtSC$STB[, -c(1:3)], 
                     2, function(x) 39 - length(which( is.na(x))) )
        props21gt <- num1s / ns
        
    # Visualize patterns
      # Occupancy by secondary sampling event
        par(mfrow = c(3,2), mar = c( 2,2,2,2), oma = c(3,3,1,1))
        ylim = c(0, 0.5); pch = 21; bg = 'grey80'; cex = 1.5
        
          plot( props19lt, ylim = ylim, las = 1, 
                main = 'Small Striped Bass (2019)', 
                pch = pch, bg = bg, cex = cex  )
          plot( props19gt, ylim = ylim, las = 1, 
                main = 'Large Striped Bass (2019)', 
                pch = pch, bg = bg, cex = cex    )
          plot( props20lt, ylim = ylim, las = 1, 
                main = 'Small Striped Bass (2020)', 
                pch = pch, bg = bg, cex = cex    )
          plot( props20gt, ylim = ylim, las = 1, 
                main = 'Large Striped Bass (2020)', 
                pch = pch, bg = bg, cex = cex    )
          plot( props21lt, ylim = ylim, las = 1, 
                main = 'Small Striped Bass (2021)', 
                pch = pch, bg = bg, cex = cex    )
          plot( props21gt, ylim = ylim, las = 1, 
                main = 'Large Striped Bass (2021)', 
                pch = pch, bg = bg, cex = cex    )
          mtext( side =1, line = 0, 'Secondary sampling event', outer = T)
          mtext( side = 2, line = 1, 'Proportion of Occupied Units', outer = T)
        
      # Occupancy by primary sampling event
        # Small striped bass
          # 2019
            prim.props.19lt <-
              c(mean( props19lt[1:2]), 
          ...
          # 2020
            prim.props.20lt <-
              c(mean( props20lt[1:2]), 
              mean( props20lt[3:4]), 
              mean( props20lt[5:6]), 
              mean( props20lt[7:8]))
          # 2021
            prim.props.21lt <-
              c(mean( props21lt[1:2]), 
              mean( props21lt[3:4]), 
              mean( props21lt[5:6]), 
              mean( props21lt[7:8]),
              mean( props21lt[9:10]))
      ...
      # Visualize patterns
        # Occupancy by secondary sampling event
          par(mfrow = c(3,2), mar = c( 2,2,2,2), oma = c(3,3,1,1))
          ylim = c(0, 0.4); pch = 21; bg = 'grey80'; cex = 1.5
          
            plot( prim.props.19lt, ylim = ylim, las = 1, 
                  main = 'Small Striped Bass (2019)', 
                  pch = pch, bg = bg, cex = cex  )
            plot( prim.props.19gt, ylim = ylim, las = 1, 
                  main = 'Large Striped Bass (2019)', 
                  pch = pch, bg = bg, cex = cex    )
            plot( prim.props.20lt, ylim = ylim, las = 1, 
                  main = 'Small Striped Bass (2020)', 
                  pch = pch, bg = bg, cex = cex    )
            plot( prim.props.20gt, ylim = ylim, las = 1, 
                  main = 'Large Striped Bass (2020)', 
                  pch = pch, bg = bg, cex = cex    )
            plot( prim.props.21lt, ylim = ylim, las = 1, 
                  main = 'Small Striped Bass (2021)', 
                  pch = pch, bg = bg, cex = cex    )
            plot( prim.props.21gt, ylim = ylim, las = 1, 
                  main = 'Large Striped Bass (2021)', 
                  pch = pch, bg = bg, cex = cex    )
            mtext( side =1, line = 0, 'Primary sampling event', outer = T)
            mtext( side = 2, line = 1, 'Proportion of Occupied Units', outer = T)
  ...
  # mapping occupancy model results
    # -----------------------------------------------
      # reference ellipsoid
        library(proj4)
        proj4string <- "+proj=utm +zone=10 +south +ellps=WGS84 
                        +datum=WGS84 +units=m +no_defs"
        
      # select data for mapping
        map.lt19 <- filter(spp.dat,
                      Species == "STB", 
                      !Forklength == "NA",
                      !SampleDate < "2018-06-14",
                      !SampleDate > "2019-06-22")
        
      # take mean of minimum & maximum xy coordinates
        map.lt19$XMean <- rowMeans(map.lt19[, c('XMin', 'XMax')], na.rm=TRUE)
        map.lt19$YMean <- rowMeans(map.lt19[, c('YMin', 'YMAX')], na.rm=TRUE)
          # xy correspond to starting & ending river kms for each sampling unit
          
      # Convert x and y coordinates to lat & long
        xy <- data.frame(X = map.lt19$XMean, Y = map.lt19$YMean)
      ...
      # Bound map by XMin, YMin, XMAX, & YMAX
        # bounds
          xmin <- min(map.lt19$XMin) - 0.4
          xmax <- max(map.lt19$XMax) + 0.4
          ymin <- min(map.lt19$YMin) - 0.3
          ymax <- max(map.lt19$YMAX) + 0.3
          bounds <- c(left = xmin, bottom = ymin, right = xmax, top = ymax)
          
        library(ggmap)
        map.new <- get_stamenmap(bbox = bounds, 
                                 maptype="toner-background", 
                                 source="stamen", zoom=8)
        
        # Map California
          stateMap <- raster::getData('GADM', country='USA', level=2)
          ca.sub<-stateMap[stateMap@data$NAME_1 == "California",]
          
          # make polygon for the state
            ca.poly<-fortify(ca.sub, region="NAME_1")
            ca.poly <- ca.poly[order(ca.poly$order),]
            ca.poly$id<-as.factor(ca.poly$id)
            
          # plot base map and vector data with ggplot
            map.lt19$observed <- 
              as.factor(as.character(stb.lt[,1:8 == "1"]))
            ebird$species_observed<-factor(ebird$species_observed, levels=c("TRUE","FALSE"))
            
            rawEbirdMap<-ggmap(map.new, darken=c(0.7,"white"))+
              geom_map(data = ca.poly, map = ca.poly, aes(y = YMean, x = XMean, map_id = id),
                       color="black",
                       fill="gray90",
                       alpha=0.8, 
                       inherit.aes = FALSE)+
              geom_point(data = map.lt19, aes(x = XMean, y = YMean),
                         alpha=0.6,shape=16,size=0.6)+
              scale_color_manual(values=c("red","gray40"), name = "Captures") +
