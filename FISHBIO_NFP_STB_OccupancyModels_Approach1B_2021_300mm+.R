...
# changes:          
  ...
  # filter out unneeded columns
    stb.gt <- 
      cap.list.2021.gtSC$STB[, -c(1,2,3)] 
  ...
  # Explanation
    # SEPARATELY alter initial occupancy, colonization, local 
      ...
      # ones produce the best functioning model FOR STRIPED BASS LONGER THAN 
      # & EQUAL TO 300 MM FORK LENGTH
    # Note: 
    ...
    # small plots of initial occupancy, colonization, & detection probabilities
      ...
      # adjust plot margins
        ...
        # predicted detection probability by sampling effort
          ...
          # plotting
            plot(pred.gt21$event, 
               pred.gt21$smoothed_occ, 
               ylim = c(0,1), 
                # plot probabilites along the y-axis from 0 to 1
               tck = 0.01,
               type = 'n', 
               ...
               ylab = "Occupancy",
               main = "Large (FL ≥ 300 mm) striped bass occupancy in 2021", 
               axes = F)
          ...
          # naive estimate of occupancy #
            # calculate proportions of occupied sites
              nums21gt <- apply(cap.list.2021.gtSC$STB[, -c(1:3)], 
                        2, function(x) length(which( x > 0)))
              ns21gt <- apply(cap.list.2021.gtSC$STB[, -c(1:3)], 
                        2, function(x) 39 - length(which( is.na(x))))
              props21gt <- nums21gt / ns21gt
          ...
          # plot points depicting estimates of actual & "naive" 
            ...
            # "naive" occupancy (doesn't account for detection %)
              points(pred.gt21$event, 
                 c(mean( props21gt[1:2]), 
                   mean( props21gt[3:4]), 
                   mean( props21gt[5:6]), 
                   mean( props21gt[7:8]),
                   mean( props21gt[10:11])), 
          ...
          # confidence intervals for "naive" estimates
            segments(pred.gt21$event, 
                   # specify where upper limit will be sourced
                   pred.gt21$smoothed_occ + pred.gt21$SE,
                    # upper
                   pred.gt21$event, 
