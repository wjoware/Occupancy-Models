...
# changes:          
  ...
  # load R files from code developed by Matt & Tyler
    ...
    load("~/OneDrive - ucsc.edu/FISHBIO/Capstone Project/Occupancy Models/Approach 1B/2021/OccupancyData_09082021.Rdata")

  # create the right reference data repository
    setwd("~/OneDrive - ucsc.edu/FISHBIO/Capstone Project/Occupancy Models/Approach 1B/2021")
  ...
  # filter out unneeded columns
    stb.lt <- 
      cap.list.2021.ltSC$STB[, -c(1,2,3)] 
  ...
  # convert counts to 0s & 1s for detection
    ...
    sum(na.exclude(stb.lt))
      # calculate the total number of sampled striped bass = 10
    ...
    # apply NAs from the compilation of all NA positions to each
      # observation covariate
        observation.covariates.2021.unscaled$flow[allNAs] <- NA
          # flow
        observation.covariates.2021.unscaled$temp[allNAs] <- NA
          # temperature
        observation.covariates.2021.unscaled$effort[allNAs] <- NA
          # sampling effort
        observation.covariates.2021.unscaled$turbidity[allNAs] <- NA
          # turbidity
        observation.covariates.2021.unscaled$conductivity[allNAs] <- NA
          # conductivity
    ...
    # ensure striped bass data is a matrix
      str(stb.lt)
      stb.lt <- as.matrix(stb.lt)
      str(stb.lt) 
        # conversion from a dataframe to matrix facilitates "NA" handling
    ...
    # check whether NAs are now aligned
      ...
      length(allNAs) # all NA locations
        # both are 30 values long, indicating that NAs from each are aligned
  ...
  # construct site covariates for occupancy models from sampling days
    # replace NAs for sampling date with column means 
      ...
      day2.0 <- matrix(NA, nrow = 39, ncol = 10)
    ...
    # develop the output that shows model comparisons
      ...
      # Export dataframe of method 2 model results to Excel file
        write.xlsx(mod.table,
                 file = "C:/Users/wjowa/OneDrive - ucsc.edu/FISHBIO/Capstone Project/Occupancy Models/Approach 1B/2020/2019_STB<300mmFL_OccupancyModel_Results.xlsx",
...
# Visualize model results
  ...
  # Plot initial occupancy probability  & colonization probabilities across years
    ...
    # use predict function get initial occupancy, colonization, &
      ...
      # form dataframe for model predictions
        ...
        E.occ  <- predict(dm9, type = "psi", nd)
        E.col <- predict(dm9, type = "col", nd)
        E.det <- predict(dm9, type = "det", nd)
  ...
  # Plotting method II
    # small plots of initial occupancy, colonization, & detection probabilities
      ...
      # calculate mean predicted occupancy among sampling events
        boot_mod.lt21 <- nonparboot(dm4, B = 5)
          # 5x boostrap (nonparametric) for the most parsimonious model for 
            # SE calculation
        boot_mod.lt21
          # view boostrapped model results
            
        pred.lt21 <- data.frame(event = 1:5, 
          # "event 1:4" represents each primary sampling event
            smoothed_occ = smoothed(dm4)[2,], 
              # smooth the curve for model estimates by column
            SE = boot_mod.lt21@smoothed.mean.bsse[2,] )
              # extract basis set supersition error from 
                # smoothed model estimates by column
  ...
  # comparison of predicted occupancy & 
    # occupancy based on detections, or "naive" occupancy
      ...
      # plotting
        plot(pred.lt21$event, 
          pred.lt21$smoothed_occ, 
          ylim = c(0,1), 
            # plot probabilites along the y-axis from 0 to 1
          tck = 0.01,
          type = 'n', 
          ...
          xlab = "", 
          ylab = "Occupancy",
          main = "Small (FL < 300 mm) striped bass occupancy in 2021", 
          axes = F)
        ...
        # naive estimate of occupancy #
          # calculate proportions of occupied sites
            nums21lt <- apply(cap.list.2021.ltSC$STB[, -c(1:3)], 
                          2, function(x) length(which( x > 0)))
            ns21lt <- apply(cap.list.2021.ltSC$STB[, -c(1:3)], 
                          2, function(x) 39 - length(which( is.na(x))))
            props21lt <- nums21lt / ns21lt
        ...
        # plot points depicting estimates of actual & "naive" 
          ...
          # "naive" occupancy (doesn't account for detection %)
            points(pred.lt21$event, 
                 c(mean( props21lt[1:2]), 
                   mean( props21lt[3:4]), 
                   mean( props21lt[5:6]), 
                   mean( props21lt[7:8]),
                   mean( props21lt[10:11])), 
          ...
          # confidence intervals for "naive" estimates
            segments(pred.lt21$event, 
                   # specify where upper limit will be sourced
                   pred.lt21$smoothed_occ + pred.lt21$SE,
                   # upper
                   pred.lt21$event, 
